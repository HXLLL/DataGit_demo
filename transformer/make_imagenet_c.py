...
imgs = list(os.listdir(sys.argv[1]))
...
def trans(f):
    ...
    img = trans_func(img, level)
    cv2.imwrite(f_dir, img)

if __name__ == "__main__":
    with Pool(processes=12) as mp, tqdm(total=len(imgs), desc=f"{method} Transforming") as bar:
        res = []
        def trans_callback(_):
            bar.update(1)
        for img in imgs:
            res.append(mp.apply_async(trans, [img], callback=trans_callback))
        for a in res:
            a.wait()
